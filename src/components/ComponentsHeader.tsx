...
export default function ComponentsHeader() {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between h-14 md:h-16 px-4 md:px-6 border-b-3 border-[#2a2a2a] bg-white">
      {/* Left: Sidebar Trigger + Breadcrumbs */}
      <div className="flex items-center gap-3">
        {/* Brutalist styled sidebar trigger */}
        <SidebarTrigger className="bg-white hover:bg-[#f0eadf] border-2 border-[#2a2a2a] text-[#2a2a2a] h-9 w-9 rounded-lg flex items-center justify-center cursor-pointer shadow-[2px_2px_0px_#2a2a2a] hover:translate-y-[-1px] hover:translate-x-[-1px] hover:shadow-[3px_3px_0px_#2a2a2a] active:translate-y-[1px] active:translate-x-[1px] active:shadow-[1px_1px_0px_#2a2a2a] transition-all" />

        <div className="h-6 w-[2px] bg-[#2a2a2a]/20 mx-1" />

        {/* Dynamic Uppercase Breadcrumbs */}
        <nav className="flex items-center gap-2 font-mono text-[10px] md:text-[11px] font-black uppercase tracking-widest text-zinc-500">
          <Link href="/" className="hover:text-wtf-orange transition-colors">
            TWEENLABS
          </Link>
          <span className="text-zinc-400 font-bold">&gt;</span>
          <Link href="/components" className="hover:text-wtf-orange transition-colors">
            COMPONENTS
          </Link>
          {currentAnim && (
            <>
