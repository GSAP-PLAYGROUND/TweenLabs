...
As you scroll down, colorful cards move horizontally across the screen. Cards enter from the bottom, slide sideways, and exit off the top — creating a premium horizontal scroll experience.
...
```bash
npx tweenlabs@latest add HorizontalCards
```
...
3. In your project, create a new file: `src/components/HorizontalCards.tsx`
...
|------|------|---------|-------------|
| `cards` | `Array` | Required | Array of card data (title, description, color). |
| `speed` | `number` | 1 | Horizontal scroll speed multiplier. |
